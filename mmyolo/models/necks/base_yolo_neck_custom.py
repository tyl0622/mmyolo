...
class BaseYOLONeckCustom(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        for idx in range(len(in_channels) - 1, 0, -1):
            # CARAFE上采样
            # self.upsample_layers.append(self.build_upsample_layer(make_divisible(out_channels[idx], self.widen_factor)))
    ...
    def forward(self, inputs: List[torch.Tensor]) -> tuple:
        ...
        # # outs = [upsample(inner_outs[0])]

        # outs = [inner_outs[0]]
        # results = []
        # results.append(self.out_layers[0](outs[0]))
        results = []
